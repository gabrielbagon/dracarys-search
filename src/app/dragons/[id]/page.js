...
export default function DragonDetailsPage() {
    const searchParams = useSearchParams(); // Use este hook para capturar parâmetros no client
    const dragonId = searchParams.get('id'); // Extrai o ID da URL
    ...
    useEffect(() => {
        ...
        async function fetchDragon() {
            try {
                ...
                if (!res.ok) throw new Error('Erro ao buscar os detalhes do dragão.');
